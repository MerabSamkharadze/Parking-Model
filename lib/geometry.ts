...
export function layout(cfg: FacilityConfig): Layout {
  ...
  // every zone needs a shaft of its own (DECISIONS C2/E2): with fewer lifts than
  // shuttles per level the corridor is not split, or a zone could never be served
  let zones = Math.max(1, Math.floor(cfg.shuttlesPerLevel));
  if (zones > Math.max(1, Math.floor(cfg.lifts))) {
    zones = Math.max(1, Math.floor(cfg.lifts));
    warnings.push(`${cfg.shuttlesPerLevel} shuttles per level need ${cfg.shuttlesPerLevel} lifts; running ${zones} zone(s)`);
  }
